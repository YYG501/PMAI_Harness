"""Single source of truth for active work stage metadata.

六步重构（office-hours 收敛）后的状态机：
- 「① 项目底座」是项目级、init 时建（PRODUCT-STATE / DESIGN / 主原型），**不是 per-req stage**。
- 活跃工作生命周期收敛成 4 个阶段：
    1 设计      —— `/design` 产出模块三件套，`spec.md` 是 build 锚点
    2 build     —— `/build` 对着模块 `spec.md` 在 prototype/ 建
    3 复审      —— 覆盖审计 / 视觉门 / 行为审 + PM 体验迭代
    4 沉淀      —— `/close` 更新 PRODUCT-STATE / PRODUCT-RULES / 模块规格并收尾
...
#
# 当前只有第 1 步「设计」有法定文档产物（spec.md）；
# build / 复审 的"产出"是 prototype 代码 + 三道审证据，无单一 .md 前置文件
# （闸门由 PM 验收把守，不靠文件存在性）；
# 沉淀 的产物（PRODUCT-STATE 更新 / merge / 按需 PRD）由 close 流程把守。
