...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/measure-tthw.sh smoke [--project-name NAME] [--target-dir DIR] [--background TEXT]
...
run_smoke() {
  ...
  started=$(date +%s)
  if ! bash "$FRAMEWORK_DIR/scripts/init-project.sh" "$project_name" "$target_dir" "$background" >"$log_file" 2>&1; then
