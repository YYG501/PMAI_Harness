...
# 每档对应一组 paragraph，AI 在 build 阶段读这段做实现指引；PM 看这段
...
DEPTH_GUIDANCE = {
    "prototype": [
        ...
        ("演示路径", "默认仅主路径（happy path）。分支路径、edge case 等 PM 没在模块 spec 明确写入就不实现。"),
    ],
    "system": [
...
# 文档输出深度 prose 段落（控制 build prompt 工程内容的展开深度）。
# custom 档不预设（等真实 custom 项目实证后再写规则）。
DOC_DEPTH_GUIDANCE = {
    "prototype": [
        ("强制引用规则", "上游已定义的类型/接口/函数签名/产品行为禁止重写——直接写「参见 docs/modules/<模块>/spec.md」或对应代码文件。"),
        ("实现规格", "只列「差异点 / 复用点」，不再贴完整签名。"),
        ("约束与易错", "默认精简；视觉规范细则沿用 docs/DESIGN.md，不展开。"),
        ("工程层验收", "只列主路径 happy path，不展开错误/空/部分态。"),
    ],
...
def render_template(schema: dict, mode: str) -> str:
    ...
    out.append("### 实现深度指引")
    out.append("")
    out.append("> 以下是 build 阶段 AI 写代码时的深度参考。PM 可手改任意条；")
    ...
    out.append("### 文档输出深度指引")
    out.append("")
    out.append("> 以下是 build prompt 生成时的工程内容深度参考。")
    ...
    if DOC_DEPTH_GUIDANCE[mode]:
        ...
        out.append("_PM 填_：本档下 build prompt 工程内容的深度参考。")
...
def _render_custom_template() -> str:
    ...
    out.append("> AI 在 build 阶段读这段做实现指引——写得越具体，AI 跑偏概率越低。")
    out.append("")
    out.append("**原型根目录（PM 必填）**：`<改成实际路径，例：prototypes/ 或 src/ 或 apps/web/src/>`")
    out.append("")
    out.append("### 代码组织")
    ...
    out.append("### 实现深度指引")
    out.append("")
    out.append("_PM 填_：build 阶段 AI 写代码的深度参考，自然语言描述即可。")
