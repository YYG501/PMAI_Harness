...
**下次接任者要知道:**
- 读 `docs/归档/完成/设计-PM手动新窗口执行.md`，重点 §3 架构 + §4.1/§4.2 改动 + §11 测试 + §13 风险
...
**🚨 已被 v4（PM 手动开新窗口 + 并行原生）取代。** 详见 `docs/归档/废弃/设计-Superset独立Claude执行.md` 顶部 deprecated banner。
...
**Status (2026-04-25):** 🟢 **Plan 收敛完成**，未实施（已被 v4 取代）。详见 `docs/归档/废弃/设计-Superset独立Claude执行.md`。
...
**下次接任者要知道:**
- 读 `docs/归档/废弃/设计-Superset独立Claude执行.md`，重点 §2 决策快照 + §8 实现顺序 + §10 v3 复用矩阵
- v3 plan (`docs/归档/废弃/设计-并行任务执行.md`) 已 deprecated 但保留作章节复用源
...
详 `docs/归档/废弃/设计-并行任务执行.md` 顶部 deprecated banner。
...
**Status (2026-04-25):** 🟡 **Plan 收敛完成，实施暂停往后放**。设计文档 `docs/归档/废弃/设计-并行任务执行.md` 已可作为 Phase A 输入，但 PM 决定先不动手。Phase A kickoff 时按 §10.1 TODO-A1~A11 逐条跑，无需重新讨论方向。

**Depends on / blocked by:**
- ~~north star 抉择~~ ✅ **2026-04-24 PM 选 (a) 并行优先**（v2 降级），尽管 /autoplan 6 路声音反对
- ~~`ScheduleWakeup` 在业务项目里是否可用~~ ✅ **2026-04-24 POC 通过**（见 `docs/归档/废弃/设计-并行任务执行.md` §9.1）
...
- **加固硬约束**：实施中必须落地 G1-G15 共 15 条加固（见 `docs/归档/废弃/设计-并行任务执行.md` §11），覆盖 /autoplan + plan-eng-review 全部 critical issues
- **测试硬约束**：Phase A 完工前 T1-T11 必须全绿（§11.5）

**下次接任者要知道:**
- **从哪里恢复**：读 `docs/归档/废弃/设计-并行任务执行.md`，重点 §10.1 (Phase A todo) + §11 G1-G15 + §11.5 T1-T11。无需重新评审 north star/Q3/Q5/Q6
