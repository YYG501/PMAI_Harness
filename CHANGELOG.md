...
PM-AI-Harness 生成器仓的演进记录。本文件**只记影响已安装框架或消费仓生成入口 / 运行行为的改动**，主要包括 `scripts/` / `skills/` / `templates/` / `hooks/`；框架源码不会复制进消费仓。`tests/` / `docs/归档/` / `RUNTIME.md` / `INVARIANTS.md` 等仅生成器仓内部使用的文件不单独记。
...
- `fix(hooks)`: Host 配置原子事务把文件身份从 device/inode 扩展为同时绑定 size 与纳秒 mtime，避免 Linux 快速复用 inode 时把同名重建的 foreign stage 或 backup 误认成本轮文件；准备、提交、回滚和清理统一使用同一身份协议，身份不一致时保留外来文件并失败关闭。
...
- `fix(spec-writing+lark-review)`: **规格 authority 与飞书跨会话恢复统一闭环。** spec-writing 现在同时支持模块 `spec.md` 和 `docs/modules/<功能>.md` 扁平功能规格；手动入口命中 `ready_to_build / building / iterating / final_check` 时不再直改已批准依据，而是分别返回 design 或当前 build。Lark review 先只读定位目标和恢复既有证据，再校验 Proposal；`required / invalid` 不创建新批次、不做 authority 写入。新增 `list-resumables` 扫描 main 与 attached worktrees，排除已 handoff 和带完成收据的 checkpoint 批次，status 在 Proposal 与普通 active work 之前恢复；显式传入仓根时先规范化 macOS `/tmp` 等合法只读路径别名，不把空评审现场误报为恢复失败。main handoff bundle 新增机器 phase：产品级按 `proposal → design → lark_review → closed`，模块级按 `design → lark_review → closed`；Proposal 生效提交、design 绑定规格的权威提交和 fresh checkpoint 各推进一步，route 只作来源审计，跳级、倒退和证据漂移失败关闭。

- `fix(workflow)`: **close 后修改按影响重新分流，并把每次完整改造隔离成独立工作轮次。** 已 close 模块的错字、单文案、局部样式、常量和规格内小缺陷可直接走 `/pmai-quick-fix`；产品行为、规格或验收变化重新进入 `/pmai-design → /pmai-spec-writing → /pmai-build`，产品方向变化则先回 `/pmai-proposal`。新轮次生成唯一 work id，验收、finalize、landing 和文档影响地图固定到包含该 id 的 `build.audit_dir`，不再复用上一轮 delta、证据或游标；旧确定性 work id 和模块级 audit 目录继续用于原合同恢复。final currentness 现在分别校验初始 design hash 与 accepted-delta 最终 hash，合法的单个或多个 delta 不再提前撞上错误比较。新 `source_hash_version=2` 让 PRODUCT-STATE、TODO 和模块索引继续进入上下文但不单独使设计过期；模块规格、输入证据、PRODUCT、PRODUCT-RULES、DESIGN、项目级决定和 project.yml 仍保持严格 currentness。没有版本字段的进行中 ready/build 保留 v1 全量范围，只有重新回到 design 后的下一次批准才升级。
...
- `fix(build+lark-review)`: **active build 反馈按影响守住产品权威边界。** 评审入口先按飞书文档身份唯一定位本地规格，并通过 `resolve-target` 将 active build 自动绑定到真实工作副本。当前模块与当前任务内、且不改变对象关系、业务规则、权限模型或关键任务路径的小调整，统一写为带 PM / 飞书审批证据和影响声明的 `scoped-adjustment`；seal 前必须在 `active_build_delta` 固化当前 work、批准 source hash、design revision、新口径、影响面和批次来源，且不能同时新增或替代稳定模块决定。Build 通过 lark-review 与 apply 共用的 sealed validator 重放 B/L/R、原生快照、resolutions、T、覆盖/预览账本、ready token 与最终 doc/revision/正文投影；手拼极简回执、路径 symlink、CLI 口径漂移和只有飞书发布 frontmatter 变化均失败关闭。飞书同步稳定后只能让当前模块 `spec.md` 进入 authority checkpoint，提交必须精确绑定 `spec.md + .work-meta.json`，并承接上一权威提交、直接接在 add-delta 时的 HEAD 后，核对 pack 文件摘要、Git blob 与当前合同，其它产品文档不能借 delta 混入或替换既有候选历史。产品级或模块模型变化不再 seal / apply 旧飞书批次，也不降级成 accepted delta：replan 后把旧批标为只读 `handed_off`，并把正文、评论、T、resolutions 与候选快照复制到 main 的持久交接包；Proposal / design / status 可跨会话枚举，旧工作环境退役后仍能恢复，多个模块级交接会逐条给出可执行的 design 动作。上游生成新权威版本后同步同篇飞书并 fresh collect，最终 checkpoint 才关闭对应交接包。历史 `term / role / product-behavior` delta 继续可恢复，新 scoped delta 缺 attestation、审批证据、authority hash transition 或内容绑定时失败关闭。
