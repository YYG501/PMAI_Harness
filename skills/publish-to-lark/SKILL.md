...
未配置类型时，PM 必须同时传 `--target-token` + `--target-kind` + `--title`，否则 skill 报错退出。模板见 `templates/lark-publish.json.tmpl`。
...
| `.claude/lark-publish.json` 存在 或 命令行参数齐全 | `配置缺失。复制 templates/lark-publish.json.tmpl 到 .claude/lark-publish.json 并填 token；或手动传 --target-token + --target-kind + --title` |
