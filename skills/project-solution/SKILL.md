...
PM 主动调用，**4 个独立场景**（vp-6 后细化 frontmatter；当前 vp-2 列基础场景）：
...
> **不在 scope**：
> - greenfield 首次起新项目 → 走 `/init-project`（一气呵成 4 阶段；阶段 C 内嵌方向讨论按同款 `_shared/project-questioning.md` 跑）
> - req 级 analysis / PRD / spec → 走 `/req-stage-gate`
> - 老项目同步兜底 → 走 `/new-req` mini-fill（不在本 skill 范围）
...
| **B 产品路线规划** | PM 主动说"产品路线规划 / 季度规划 / 半年规划"，或老项目首次补 `docs/ROADMAP.md` | `docs/PROJECT.md` 已有；`ROADMAP.md` 历史可有可无（首次补则跳过步骤 1）| (1) **过去 roadmap 完成度回顾**（哪些 ship / 哪些砍；首次补无历史则跳过）→ (2) 产品路线（新里程碑）→ (3) roadmap（新 req 队列 + 排优先级）→ (4) 业务术语表增量（如有新业务）—— **跳过产品定位 / 用户画像 / 技术栈**（默认稳定）|
...
- `docs/PROJECT.md`（已有内容 / 或 `/init-project` 已建的空骨架）
- `CLAUDE.md`（看注入的项目背景 / 工程结构约束档位）
- `docs/代码现状档.md`（如存在 —— brownfield 场景；`/codebase-audit` 先扫码产出的 7 维度现状档）—— 存在 → 全文读，作为讨论实况语境
...
- ❌ 走 req stage / 调 `req-transition.py` / 调 `/req-stage-gate` —— 本 skill 是项目级，不占 req stage
