...
- **问题会诊入口（/pmai-meta）**：push 深想时，如果卡点不只是"怎么摆字段 / 怎么写规格"，而是**这需求到底成不成立、服务哪个目标、按什么标准取舍、帮谁做什么判断、当前方案依赖的前提是否成立、或表达停在功能层**——调 `/pmai-meta`。`/pmai-meta` 是探索段里的问题会诊入口，不是新阶段、也不是分析段落生成器：它先读资料，再一题一问，挑战前提，给出替代方向并停住让 PM 选，最后把结论降回本 skill 段②理结构。**仍走 escape-hatch**：小需求默认放行不调；一问现状就能还原真问题的，用探索内核轻推即可；闻到"目标不清 / 标准不清 / 旧范式 / 前提存疑 / 表达太功能层"，或 PM 说"帮我深想 / 不合理 / 感觉不对 / 只是复述"才调。
- **根上压测（可选旁路）**：PM 明确说"多角度看看 / 找盲区 / 让几个 agent 分别看 / 对抗分析"，或连续质疑"不合理 / 感觉不对"且单一探索视角仍卡住时，可建议调 `/pmai-meta`。由 meta 先判断已有锚点是否足够：有当前问题 / 方案 / 规格草稿就压测根因和前提；发现缺的是 PM 判断时转回一题一问。它只帮把问题模型想清楚，不替代模块结构设计、规格自检或 PM 拍板。**如果问题其实是 PMAI skill / workflow 边界，不回 design，转 `/pmai-skill-improve` 或明确停住。**
...
- **`/pmai-meta` 只作问题会诊旁路**：当 PM 想深想、质疑"不合理 / 感觉不对 / 只是复述"、多视角找盲区或反方挑战时建议使用；不自动插入每次 design，不替代探索段、模块结构设计、规格自检或 PM 拍板。skill / workflow 改造问题不属于 design，按 meta 判断后交 `/pmai-skill-improve`。
