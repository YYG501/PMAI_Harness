...
- **换高度工具（升维 ↑ / 第一性原理 ↓）**：push 深想时，如果卡点不只是"怎么摆字段 / 怎么写规格"，而是**这需求到底成不成立、服务哪个目标、按什么标准取舍、帮谁做什么判断、当前方案依赖的前提是否成立、或表达停在功能层**——调 `/pmai-meta`。`/pmai-meta` 是探索段里的对焦工具，不是新阶段：需求不真实也可以用它，往下拆基本事实和隐含假设，往上找这件事该接受哪套更高标准。**仍走 escape-hatch**：小需求默认放行不调；一问现状就能还原真问题的，用探索内核轻推即可；闻到"目标不清 / 标准不清 / 旧范式 / 前提存疑 / 表达太功能层"，或 PM 说"帮我深想 / 拔高"才调。调完**必须降回**段②理结构，不停在高空。
- **多视角压测（可选旁路）**：PM 明确说"多角度看看 / 找盲区 / 让几个 agent 分别看 / 对抗分析"，或连续质疑"不合理 / 感觉不对"且单一探索视角仍卡住时，可建议调 `/pmai-meta`。由 meta 先判断有没有明确锚点：没锚点走对焦 / 推导，有当前问题 / 方案 / 规格草稿才走多视角压测。它只帮找盲区和根因，不替 PM 拍板，也不升格成每次设计必跑门。
...
- **页面定位掰不清时**（这页是给人看任务、还是做判断？信息该堆哪些）→ 可调 `/pmai-meta` 的 UI 问法（信息 / 任务 / 判断 / 更高标准）：先定"这页服务谁的哪个判断"，再按这轮讨论的标准反推卡面 / 主操作 / 默认筛选 / 不该出现的东西。轻量、可选，思路已清就别调。
...
- **`/pmai-meta` 的压测模式只作 PM 主动旁路**：当 PM 想多视角找盲区、反方挑战或看当前方案哪里不对时建议使用；不自动插入每次 design，不替代探索段、规格自检或 PM 拍板。
