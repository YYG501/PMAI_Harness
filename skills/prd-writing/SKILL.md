...
完整列出**每个一级功能 × 每个二级功能 × 每个三级功能**对**每个角色**的权限点，用 HTML `<table>` 写法以支持 `rowspan`（一级功能跨多个三级功能行时合并）；markdown 管道表格不支持 rowspan。

> ⚠️ **飞书发布未解**：lark-cli 不识别 HTML `<table>`，权限矩阵发布到飞书会塌成纯文本；而 publish-to-lark 的管道表格合并逻辑又把「空单元格」当续行合并，与权限矩阵「空 = 无权限」语义冲突 —— 两条路都不通。权限矩阵的飞书渲染方案待定。发布前 publish-to-lark 会对 `<table>` 报警提示。
...
- 新需求场景保留"优先级"列。
- **「原型」列**：stage 3 写 PRD 时没有真实原型截图 —— §六的产物意图描述写在每个 §6.X 模块的「原型」节（文字段落），不进表格列。close-req 反向对齐时若回填真实原型素材，可加最右「原型」列嵌入 `<img name="image.png" mime="image/png" scale="1.000000" src="<token>"/>`，同一二级功能下多行用 `rowspan` 合并到首行。⚠️ **飞书发布未解**：含 `<img>` 的合并表只能用 HTML `<table>` 写，lark-cli 不识别、发布会塌成纯文本 —— 加原型列前先确认飞书发布方案（与 §八 权限清单同属待定项）。
