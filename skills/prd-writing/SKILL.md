...
- **第一步，锚定语体**：动笔前读 `references/few-shots.md` 顶部「大白话模块 spec 金标准」一节——那里的**6 条可检验风格卡 + 待办实例 + 语体转换反例表**就是模块 spec 的目标语体。模块 spec **学待办那种大白话讲法，不套 PRD 的 L1–L6 重型指纹**（套了就会写成"咨询报告 / 方法论白皮书"腔，PM 还得逐句改回来）。`references/few-shots.md` 下半部的真系统 PRD 样例**只在 PRD 模式参照，模块 spec 不模仿**。
...
- **冷读语体（必做）**：通读一遍自问——"这读起来像待办那种**大白话产品规格**，还是像**咨询报告 / 方法论白皮书**？"。若后者，标出最像白皮书的 2–3 处，对照风格卡 6 条改。必要时派 subagent 冷读：给它风格卡 6 条 + 待办实例，让它只回答"像哪一类、哪几处不像大白话"。
...
详见 [`references/few-shots.md`](./references/few-shots.md)。**先按模式选看哪段**：

- **模块规格模式 → 顶部「大白话模块 spec 金标准」一节**：6 条可检验风格卡 + 待办实例 + 语体转换反例表。模块 `spec.md` 学这种大白话讲法，**不套下面的重型 PRD 样例**。
- **PRD 模式 → 下半部「真系统 PRD 案例」**（重型风格，给研发评审 / UI 即交付契约），覆盖：
- 摘要示例（权限控制 / 探索发布两种场景）
