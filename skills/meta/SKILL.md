...
## 入口判断：先判断是不是 Office Hours

命中以下任一，就进入 Office Hours，不要直接输出结论：
...
Office Hours 的输出不是“分析段落”，必须收敛到四件事：
...
| 后台类型 | 处理方式 |
|---|---|
| 新想法 / 方向不清 | Office Hours 访谈，先问真问题和判断标准 |
...
如果找不到危险前提，就说明问题可能很小，不该继续 Office Hours；回到直接执行。
...
这些工具只在 Office Hours 会话里服务“真问题 / 前提 / 替代方向 / 下一步”。禁止把工具箱章节当成前台模板逐项输出。
