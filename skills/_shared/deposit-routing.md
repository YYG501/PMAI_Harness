<!-- 共享参考 · 沉淀的分流。单一真相源，被多个开火点 @读，各按自己重量套用：
       · /close（原 close-req·中/重档沉淀，规格定稿那一步）
       · /pmai-deposit（轻档·main 直接改的轻沉淀，dormant 但保留）
     真相源设计见生成器仓设计文档「吸收ExampleAgentProject设计方法-改造方案」§3/§5/§6
       + 「消费仓文档结构.md」（新布局：docs/ 一棵树 + modules 三件套 + inputs 自动归类）。
     原则：机制不新造——这些类的落点子设计里都有；本参考只把它们收成一处、让每个开火点复用。
     ⚠️ 瘦身改造后落点已迁到新布局：取消 requirements/ 树、决策 2 个家、附件按类型自动归类、决策+术语回写。 -->

# 共享参考：沉淀的分流（新布局）

## 一句话

一次沉淀 = 把本次产出按"类型"分流到几个**后面会被读到**的家，并顺手把新文档挂进脊柱索引。**当前产品长什么样、为什么这么拼、想做没做的、探过哪些视觉、新造的术语、下个 req 该知道的决策**——各归位，没有关键信息只躺在原地没人读。

> **新布局前提**（详见 `docs/设计/消费仓文档结构.md`）：唯一组织单位 = 功能模块（`docs/modules/<模块>/` 三件套）；取消 `requirements/` 树；决策只有 2 个家（模块 `decisions.md` + 项目 `PRODUCT-RULES`）；附件按类型自动归类进 `docs/inputs/<类别>/`。
...
| **② 决策与理路（为什么这么拼）** | **跨模块 / 全局**：护城河论证 / 机制整体设计意图 / 交互咬合推导 / 演进故事 + 跨模块规则的"为什么"。**单模块**：本模块结论 + 为什么 + 否过什么 | **2 个家**：跨模块 / 全局 → `docs/PRODUCT-RULES.md`（吸收原 `docs/decisions/`，带日期 + supersede 留痕）；单模块 → `docs/modules/<模块>/decisions.md` | AI 判属哪个家（跨模块还是单模块）+ 判门槛（有实质理路才记 / 纯微调不记），AI 提议、PM 确认。**`docs/decisions/` 已折进 `PRODUCT-RULES`、不再是落点** |
...
| **⑤ 跨 req 决策回写** | 本 req 拍的、后续 req 该知道的决策 | 同 ②：属某模块 → 模块 `decisions.md`；跨模块 / 全局 → `PRODUCT-RULES.md` | `/close` 沉淀加一问；现框架沉淀只收"整体意图"、单条决策会漏在已关 req 里——补这一问让它沉进 `/design` 会读到的家（跨 req 记忆 · 写侧）|
| **⑥ 术语回写** | 本 req 新定义的概念 / 术语 | `docs/PRODUCT.md` 业务术语表 | `/close` 沉淀加一问；下个 `/design` 开场会读到业务术语表，PM 不用重复说（跨 req 记忆 · 写侧）|

> **②⑤ 合流说明**：②（沉淀本轮理路）和 ⑤（回写后续 req 该知道的决策）落点完全相同（2 个家），只是触发问法不同——②问"本轮有没有实质理路要留底"，⑤问"本 req 拍的决策里有没有后续 req 该知道的"。实现时一道沉淀问把两者一起收。
...
> 详细类别集 + 安全预检 + 迁移接口见 `docs/设计/消费仓文档结构.md` §四 / §五。caller 行为约定见 `_shared/pm-view/attachments-upload.md`（瘦身后落 `inputs/<类别>/`、不再 worktree 后置）。

## 各开火点怎么套用（重量不同，分流相同）

- **`/close`（中 / 重档）**：规格定稿那一步，本就在写 PRODUCT-STATE（①）+ 模块 `spec.md`。在它沉淀候选清单里**加 ② 决策 / 理路类识别**（属某模块 → 模块 `decisions.md`；跨模块 → `PRODUCT-RULES`）、**③「推下个需求」自动入 TODO**、**④ 本 req 探索的 mock 登记**、**⑤ 跨 req 决策回写**（一道问和 ② 合收）、**⑥ 新术语回写 `PRODUCT.md` 业务术语表**。属于完整沉淀仪式（PM 逐候选拍 + 总审 diff）。
...
给 PM 的话只用 PM 视图语言：「现状 / 规则 / 遗留各归位了」「这轮拍的决策记进 X 模块了 / 记进项目规则了」「新词「Y」补进术语表了」「材料归类到「竞品调研」里了」「探的几版视觉留在看版里了」。**不出现**"分流 / manifest / featured / 索引展开层 / 冻结档 / inputs / stage_prefix / 决策的 2 个家"等内部词——这些是后台机制，PM 永不用记。
</content>
