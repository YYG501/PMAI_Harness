...
它不是第二条工作流。design 收口时做轻量扫描；PM 看 build 结果期间不做完整扫描，明确请求定稿后才对冻结 commit 完整扫描并生成验收就绪快照；final_check 只校验快照，landed 后文档编译继续沿用同一口径。机械缺口自动处理，只有产品模型岔路、不可逆动作或要改变 PM 已确认方向时才打断 PM。
