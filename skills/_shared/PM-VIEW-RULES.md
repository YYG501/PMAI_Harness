...
**关键变化（相对旧格式）**：
- 弃三列表「二级功能 / 三级功能 / 使用角色」 — 单行三列容易凑出无意义的拆层（"产品详情-基本信息 / 信息块展示" 把同一件事拆两层）
- 角色限定改成标题下一句话引用块
- 字段口径独立成表，不再混在数字编号项的子列表里

如有跨子功能聚类需要，标题用 "功能 A · 功能 B · 功能 C" 列举即可，不再强制拆"二级 / 三级"。

### 5.2 业务规则编号项写作约束（硬规则）

**核心原则**：每条业务规则只回答"产品在业务层提供什么"——业务命题、业务规则、角色权限、可见范围。**不描述 UI 长什么样**——UI 形态是「📐 产物预览」章节的职责。
...
| 字段值口径 | "总席位 = 跨所有 active 证的产品级总席位 sum" | 进本节 5.1 的「字段口径」表（业务语言）|
...
- [ ] 角色限定一句话紧接标题，不写表格
- [ ] 业务规则编号条目都通过"换 UI 还成立"测试（§5.2 末尾的判别简则）
- [ ] 业务规则无 UI 排版词：灰底 / 圆角 / 2 列 / 上下结构 / 第 N 行 / 跨 X 列 / 左右排列 / 顶部 / 底部 / 副标题 / 主按钮 / 二级按钮
- [ ] 业务规则无 UI 控件类型词：segmented control / kebab / tree connector / 折叠区 / sticky / overlay
- [ ] 业务规则无样式参照词："与 X 对齐 / 同款 / 一致样式"
- [ ] 业务规则无 how（实现路径）/ why（设计意图）/ 字段口径
- [ ] 字段口径若有 → 独立成表；「计算口径」列用业务动词（累加 / 统计 / 取 / 减）
...
1. 用户进入产品详情页时，需要在第一时间得到本产品的"宏观状态画像"——即"我这个产品有多少额度、用掉多少、还有多少、覆盖到多少部门"
2. 产品概览是产品级聚合视图，不区分许可证；许可证维度的细分是 Tab 1 内部视图切换的职责，不在本节范围
3. 部门管理员看到的字段值按其管辖范围聚合：不在管辖范围内的部门额度、不计入"已用席位"和"部门覆盖数"
4. 产品概览的字段值与 Tab 1「部门视图」根节点行的字段值口径完全一致——同一个数字只允许有一个真实来源

**字段口径**：

| 字段 | 业务含义 | 计算口径（业务语言）|
|---|---|---|
| 产品 ID | 产品的系统标识 | 直接取产品记录上的标识值 |
| 来源许可证数 | 当前产品挂在多少张许可证下 | 统计本产品下所有许可证数量（含已失效）|
| 总席位 | 本产品在已激活许可证下的总配额 | 累加所有已激活许可证的产品级总席位 |
| 已用席位 | 本产品下已分配给用户的席位数 | 累计所有挂在已激活许可证下的开通记录（含直接开通的用户 + 各部门内开通的用户）|
| 剩余席位 | 本产品下还能再分给用户的席位数 | 总席位 减 已用席位 减 已分配给各部门但未用的额度 |
| 部门覆盖数 | 本产品额度被分配到的部门数 | 统计在本产品下持有任意非零额度的部门数（含子部门）|
| 产品描述 | 本产品的简短业务描述 | 直接取产品记录上的描述字段 |
```

正例与反例的差别：
- 标题改为业务名（"产品概览"），去掉"顶部信息块"这种 UI 元素称呼
- 业务规则只说**业务命题**：用户在该场景下需要什么、产品提供什么、不同角色看到的范围有什么差别、与其他位置的口径一致性
- **完全不提**：灰底信息块 / 2 列 grid / 字段排几行 / 按钮在哪边 / 与哪个页面的样式对齐——这些 UI 形态去看「📐 产物预览」章节
- 字段口径独立成表；「计算口径」列用"累加 / 统计 / 取 / 减"等业务动词
- 全节无中英混杂工程词、无反引号代码引用

**判别"是不是 UI 描述"的简单 test**：把这条规则套到一个完全不同的 UI 实现上（如换成纯文字列表、或者换成两栏布局）——业务上还成立吗？
- 反例编号 1：换成"两栏布局"就不成立了 → 是 UI 描述
- 正例编号 1："用户需要看到宏观状态画像"——任何 UI 都成立 → 是业务规则
