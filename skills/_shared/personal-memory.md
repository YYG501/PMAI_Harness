...
4. 表达同一判断的候选合并；互相冲突的候选不能同时使用，按当前项目事实和最新纠偏处理。
5. 使用过滤后所有仍有独立检查价值的经验，把它们转成后台检查，不机械变成 PM 确认题。

正常召回不设置固定条数。脚本按相关性排序、折叠高度重复候选，并用可配置的字符预算保护上下文；固定候选数量只用于数据库异常时的底层安全保护，不决定正常情况下使用几条。如果多条有效经验经常超出预算，说明记忆过于碎片化，应合并经验，而不是长期扩大条数上限。
