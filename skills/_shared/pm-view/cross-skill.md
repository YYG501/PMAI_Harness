...
2. **`docs/DESIGN.md` 是视觉单一来源**：
   - `design` / `new-req`：全文必读，用于范围与视觉草图约束。
   - `build`：动手前全文必读。
   - `review` / 视觉门：全文必读并对截图校验。
   - `close`：只在视觉规范类反馈需要沉淀时修改。
...
6. **PM 反馈分流只在 close 沉淀**：build / review 期间的 PM 反馈先用于改 demo；需要长期沉淀的内容在 close 里按视觉、术语、跨功能规则、模块规格四类分流。
