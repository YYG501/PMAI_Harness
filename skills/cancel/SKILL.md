...
name: pmai-cancel
description: |
  放弃当前进行中的模块工作：确认后不合并当前工作分支，清掉模块的活跃状态，并把关联 worktree/branch 标记为待清理。
---

# /pmai-cancel

> 本 skill 是当前工作的退出口。它不判断需求是否“正确”，只在 PM 明确说这轮不要继续时，把活跃状态清掉，避免后续 `/pmai-status` / `/pmai-next` 继续把它当进行中工作。
>
...
- 当前模块已有 `.req-meta.json` 活跃状态，需要清掉。
- 不用于完成收尾；完成并沉淀走 `/close`。
...
```text
即将放弃当前工作：$ACTIVE_REQ（当前阶段：$ACTIVE_REQ_STAGE_NAME）
...
2. 删除当前模块在 main 上的 `.req-meta.json`（如果存在），清掉“正在做”的状态。
...
- 不手动删除 worktree/branch；只通过 `cancel-req.sh` 写 pending，再由 `cleanup-pending-worktrees.sh` 清理。
