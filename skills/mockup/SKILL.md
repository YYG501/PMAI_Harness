...
从讨论结论里挑出**真正的视觉/布局岔路**，定几个变体方向。每版围绕一条想试的方向，比如：
...
**用 AskUserQuestion 让 PM 定这轮出几版**（默认 3，重要界面到 5-8；简单界面 1-2 版意思一下就够）。**变体之间要真不一样**（不同方向，不是同一版换个颜色），PM 才挑得出取舍。把"谁看、放什么字段、哪些信息一组"连同每版方向整理清楚，作为生成依据。
...
- **挑某几版的某几块拼**：记下"取 A 的导航 + B 的抽屉"这类结论，作为回 `/pmai-design` 写规格的依据。
