...
name: pmai-deposit
description: |
  轻档轻沉淀：PM 在 main 上聊定直落 / 不进 req 的轻量工作做完后，把成果按四类归位进项目底座
  （现状→PRODUCT-STATE / 理路→项目决策记录 / 遗留→TODO / 探索变体→mocks 看版），单独 commit。
  PM 显式「沉淀一下」入口，也承接 AI 在收敛点的主动提议。轻量同构于 close-req 的沉淀，但只在 main 触发、重量随产出缩放。
---

# /pmai-deposit

> **这是什么**：分档运行里**最轻那一档**（main 直接改 / 聊定直落、跳过 req）的沉淀开火点。让轻档产出也有一个地方收口，不再"飘着没进库 / 真相源乱 / 决策不进库 / mock 找不回"。
> **和 close-req 的关系**：同一套四类分流（@读 `_shared/deposit-routing.md`），但 close-req 是六步最后一步的完整仪式（在 req worktree 里做、PM 逐候选拍），本 skill 是 main 上的轻量同构（重量随产出缩放）。
...
- **不适用**：要起完整需求 → `/pmai-new-req`；只是在 main 改一处文件 → `/pmai-quick-fix`；在 req 里推进 → `/pmai-next`。
...
**@读 `skills/_shared/deposit-routing.md`**，按四类把本次产出分流，**重量随产出缩放**——多数轻档只动 ①：
...
轻沉淀直接写 main 上的底座文件。main 有写保护（`check-branch.sh` GATE 3），所以走 marker 门控的合法写入口：

```bash
# 1) 设 marker（让 GATE 3 放行沉淀四类落点到 main；marker 限定 allowlist，blast radius 仅这几个家）
mkdir -p "$REPO_ROOT/.runs"
rm -f "$REPO_ROOT/.runs/deposit-in-progress"   # 先清任何陈旧 marker（防上次崩溃残留），再重设
touch "$REPO_ROOT/.runs/deposit-in-progress"
```
...
```bash
# 只 add 本次沉淀实际动的文件（逐个列，不用 git add -A）
git -C "$REPO_ROOT" add docs/PRODUCT-STATE.md   # + 本次实际动的其它落点
git -C "$REPO_ROOT" commit -m "沉淀: <一句话本次归位了什么>"

# 清 marker（must——commit 后 / 任何中断都要清）
rm -f "$REPO_ROOT/.runs/deposit-in-progress"
```

### 步骤 5：回执

给 PM 一行回执（**PM 视图语言，不出内部词**，见 deposit-routing §PM 话术纪律）：
...
- **防腐铁律不破**：轻沉淀是 PRODUCT-STATE 的合法原子写入口之一（与 close-req 并列），不是退回"随手改活文档"；写入仍是收敛点触发的原子动作。
