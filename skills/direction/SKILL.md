...
|---|---|---|---|
| **方向重定** | PM 说“方向偏了 / 要重定 / 定位不对 / 老板或客户给了新方向” | `PRODUCT.md` 已有内容 | (1) 先问旧方向哪里失效 / 新方向从哪来 → (2) 产品定位 → (3) 用户画像 → (4) 关键边界 / 技术栈是否变化 → (5) 业务术语表 → (6) 刷新 TODO 待办池 |
| **路线规划** | PM 说“产品路线规划 / 季度规划 / 半年规划 / 接下来做什么 / 整理待办” | `PRODUCT.md` 已有；`TODO.md` 可有可无 | (1) 问 PM 现在想做什么，记进 TODO 待办池 → (2) 如出现新业务概念，补业务术语表。默认跳过产品定位 / 用户画像 / 技术栈 |
...
**路线规划**：默认不重写产品定位 / 用户画像 / 技术栈；只有 PM 明确说这些也变了，才升级为“方向重定”。
...
> **本 skill 不写 PRODUCT-STATE**：direction 不是 PRODUCT-STATE 的 sanctioned 写口（防腐铁律只认 `/pmai-build-close` + `/pmai-record`）。冻的决策记录靠 `/pmai-design` 进场按需扫 `docs/decisions/` 发现。
