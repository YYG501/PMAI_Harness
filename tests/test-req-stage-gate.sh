...
test_stage67_steps_replaced() {
  ...
  _assert_contains "$REQ_STAGE_GATE_SKILL" "All satisfied → prose 头部 + AskUserQuestion" "new confirmation gate (picker style)" || return
  _assert_contains "$REQ_STAGE_GATE_SKILL" "Stage 6 task 执行 → 7 req close" "prose 头部 (picker style)" || return
