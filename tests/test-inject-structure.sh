...
# -----------------------------------------------------------------
# delta-3：task-spec §5 双源 prose 合并机制已随双文件塌缩删除。
# 旧 T13a/T13b/T13c 断言的「CLAUDE.md 工程结构约束 + solution.md 本轮实现深度
# 变更」prose 合并 / 冲突阻断 / framework 档分支均已不存在 —— task-spec v3 直接
# 读 implementation-design.md（HOW）。相关断言随机制删除。
# -----------------------------------------------------------------
