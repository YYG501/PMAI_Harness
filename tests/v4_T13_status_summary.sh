...
test_summary_counts_active_tasks() {
  start_test "status-view --summary counts executing, review, and pending-start tasks"
  fixture_setup

  req_dir=$(fixture_create_req "req-001" "summary" 6)
  fixture_create_task "$req_dir" "001" "running" "执行中" >/dev/null
  fixture_create_task "$req_dir" "002" "review" "待验收" >/dev/null
  ...
  if ! echo "$out" | grep -F -q "📋 task 概览: 执行中 1 / 待验收 1 / 待启动 1"; then
    _fail "summary line missing or wrong. Output:"
    echo "$out" >&2
    fixture_teardown
    return
  fi
  if ! echo "$out" | grep -F -q "⚠️ 1 个 task 待验收"; then
    _fail "review warning missing. Output:"
    echo "$out" >&2
    fixture_teardown
    return
  fi
