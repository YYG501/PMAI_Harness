...
assert_grep "模块已经 close" "close 后的规格内小修仍可走 quick-fix"
assert_grep "改模块规格、补验收路径或改一片功能" "close 后的产品变化拒绝 quick-fix"
assert_grep "当前合同" "当前合同分类存在"
assert_grep "历史档案" "历史档案分类存在"
assert_grep 'source "${PMAI_HOME:-$HOME/.pmai}/scripts/skill-preamble.sh"' "preamble 尊重自定义 PMAI_HOME"
