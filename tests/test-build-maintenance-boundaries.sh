...
test_build_modules_are_separated() {
  ...
  if grep -Eq '_lib\.lark_adapter|lark-cli|lark-review\.py' "$CONTRACT"; then
    _fail "core Build CLI must not directly depend on Lark implementation"
  elif ! grep -Eq 'lark-review\.py' "$REVIEW_ADAPTER"; then
...
test_release_gate_excludes_optional_lark_runtime() {
  ...
  if ! grep -Eq '^OPTIONAL_LARK_SUITES=\(' "$RUN_ALL"; then
    _fail "optional Lark suites should be named explicitly"
  elif ! grep -Eq 'PMAI_SKIP_OPTIONAL_LARK_TESTS=1' "$RELEASE_GATE"; then
    _fail "stable core release gate should exclude optional Lark suites"
  elif ! grep -Eq 'test-lark-adapter\.sh' "$RUN_ALL"; then
...
test_atomic_threat_model_is_explicit() {
  ...
  if ! grep -Eq 'Supported threat model \(ADR-004\)' "$ATOMIC_FILE" \
    || ! grep -Eq 'cooperative writers' "$ATOMIC_FILE" \
    || ! grep -Eq 'malicious same-user process' "$ATOMIC_FILE" \
    || ! grep -Eq 'non-cooperative writer' "$ATOMIC_FILE"; then
