...
# 建一个最小消费仓 fixture：.pm-workflow/config.yml + req-plan.md + prototype/ + task 文件
_setup() {
  T=$(mktemp -d)
  REQ="$T/requirements/active/req-001-demo"
  mkdir -p "$T/.pm-workflow" "$T/prototype" "$REQ/tasks" "$T/docs"
...
dev_server:
  ...
  ready_check: /
EOF
  echo "# 范围清单" > "$REQ/req-plan.md"
  echo "# task" > "$REQ/tasks/task-001-foo.md"
  echo "# DESIGN" > "$T/docs/DESIGN.md"
  TASK="$REQ/tasks/task-001-foo.md"
  AUD="$T/.pm-workflow/tasks/task-001-foo/audits"
}
_teardown() { rm -rf "$T"; }
...
test_resolve_ok() {
  ...
  _setup
  out=$(python3 "$AUDITS" resolve "$TASK" --repo-root "$T" 2>&1)
...
test_resolve_missing_rangelist() {
  start_test "resolve: 缺范围清单 req-plan.md → fail-loud"
  _setup
  rm -f "$REQ/req-plan.md"
  if python3 "$AUDITS" resolve "$TASK" --repo-root "$T" >/tmp/ba.$$ 2>&1; then
    _fail "缺范围清单应 fail-loud 非零"
  else
    grep -q "范围清单" /tmp/ba.$$ && pass_test || _fail "应提示范围清单缺失：$(cat /tmp/ba.$$)"
  fi
...
test_synthesize_incomplete_fails() {
  ...
  # 故意不写 behavior.json
  if python3 "$AUDITS" synthesize "$TASK" --repo-root "$T" >/tmp/ba.$$ 2>&1; then
...
test_synthesize_clean_gate() {
  ...
  out=$(python3 "$AUDITS" synthesize "$TASK" --repo-root "$T" 2>&1)
...
test_synthesize_fail_on_gate_exit() {
  ...
  if python3 "$AUDITS" synthesize "$TASK" --repo-root "$T" --fail-on-gate >/dev/null 2>&1; then
