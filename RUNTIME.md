...
- 正常用户主链路：`/pmai-init-project` 只建上下文骨架 → `/pmai-proposal` 完成产品级澄清 → `/pmai-design` 收敛模块决定并在首次定稿时生成项目建造定义 → `/pmai-spec-writing` 编译已确认规格 → `/pmai-build` 默认派发 native-child Builder → PM 看结果多轮快速修改 → PM 明确定稿 → 独立 Verifier 执行机械 final checks、独立 Judge 只读复核语义 checks 并绑定同一 evidence digest → 合入 main → 只更新实际受影响的产品真相源。child / 外部能力不可用时，主控可降级接管，但必须记录 `main-fallback` 与“非独立降级执行”；spec-writing 通常由 design 自动调用，不要求 PM 手工拼接命令。
...
- 项目建造定义：初始化时不存在；首个达到 `ready_to_build` 的 design 将 `prototype / product`、技术栈、代码入口、真实命令和 Web 能力写入 `.pm-workflow/project.yml`。后续 build 只读该文件。Design 定稿后同一轮自动进入 `/pmai-build` 预检并展示工作环境 / 构建工具确认卡，不再停在等待 PM 手动询问下一步。
...
- 文档语义：Proposal 定义产品级用户、问题、价值、边界、端到端体验、关键能力与 MVP 验证逻辑；下游用它判断模块取舍、保留的产品主张和要验证的用户结果。`spec.md` / PRD 是指导研发实现的最终目标合同；`PRODUCT-STATE.md` 描述 main 已落地现状；原型、mockup 和代码只作设计 / 实现证据。
- design 的 `ready_to_build` 状态同时记录 approved source hash、hash scope version、design checkpoint 和精确目标路径；build 开工前重新编译上下文并验证 currentness，过期依据不能继续显示或进入构建。当前 Proposal 与固定交接摘要参与 context pack 和 source hash；Proposal 换版会让旧 ready/build 依据失效。新轮次使用 `source_hash_version=2`：模块三件套、输入证据、PRODUCT、PRODUCT-RULES、DESIGN、项目级决定和 project.yml 参与 currentness；PRODUCT-STATE、TODO 和模块索引只作上下文，单独变化不判设计过期。没有版本字段的旧 ready/build 按 v1 全量范围恢复到该轮结束，显式返回 design 后下一次批准才升级 v2。普通 `designing` 遇到有效 `ready_to_build` 直接拒绝且不改合同；PM 明确要求重开时使用 `build-contract.py reopen-ready --reason ...`，active build 仍通过既有 replan 入口处理。相同 checkpoint、source hash、design revision 和目标路径的 `ready` 重试幂等成功，复用既有授权并刷新、验证 context pack；任一批准输入变化必须先显式 reopen，active build 也不能被 ready 覆盖。新 `.work-meta.json:build` 使用合同 v5：保留 v4 的双车道验收语义，但 lifecycle 和 checks 都只写一个 canonical 位置；旧 v1-v4 继续用于中断恢复兼容。
...
- 新 build 由 AI 在一张卡中展示推荐的工作环境、构建工具和两项的全部有效选择，PM 只确认这两项；外部工具候选排除当前主控对应的 profile，“当前主控后台构建（native-child）”和“当前会话直接构建（main-fallback）”始终可选，没有可用外部工具时默认推荐前者。项目类型和验收方案不显示。选择当前环境时，main 只放行合同声明的目标路径。
- 宿主面分为两级：Claude Code 使用 `/pmai-*`、Codex 使用原生 `$pmai-*`，两者保留完整 Skill、项目 Hook、lifecycle、恢复、证据和 landing 主控能力；OpenCode 只由 `/pmai-build` 选作外部 Builder。新 build 默认组合为独立环境 + 当前主控后台 native-child，Kimi Code 与 Cursor Agent 不再进入候选；历史 adapter/合同仅保留恢复兼容。Builder prompt 明确禁止调用 PMAI Skill、推进 lifecycle、写验收通过或处理 landing；主控重新采集 Git diff、检查路径并决定后续状态。新 install / upgrade / init 不再生成 Kimi Skill/managed hooks 或 OpenCode commands/config；Doctor 只报告遗留资产并保留显式清理入口。
...
- P0 Harness 证据链已落地：`INVARIANTS.md` 的 22 条生命周期、写入边界、收尾恢复和评测不变量都可追踪到 guard / deterministic test / Session Eval 状态，未知或缺失映射失败关闭。`natural-language-finalize` 使用具备完整 PMAI 初始化、Proposal、`project.yml` 和 `iterating` build 合同的脱敏 fixture；另外 17 个案例也已建立相应隔离 fixture。Harness 独立采集 baseline / final commit、文件 hash、实际修改、保护路径和 Git diff，生成绑定 case 与 runner digest 的 evidence manifest。Runner 伪成功、越界修改、Judge digest 篡改均有反例回归；本轮真实 runner 与独立 Judge 已在全部 18 个案例中通过。
- P1 运行证据闭环已实现：`skill-eval-codex-runner.py` 通过 Codex CLI JSONL 采集线程、工具、命令、生命周期、Token、耗时和失败原因，兼容真实 `turn.completed.usage` 与旧 `event_msg.token_count`，并将非致命 `item.type=error` 保留为诊断；非零退出会纳入 stderr。新增临时导航 context pack，限制重复读取和无关仓库扫描；新增 selector 驱动的流式事件采集、进程组 timeout 和仅记录事件元数据的 telemetry；完整结果 marker 后的 reconnect / transport 断线可交给独立 Judge 复核，业务错误不会被恢复。`skill-eval-readonly-judge.py` 只读复核 manifest、保护路径、预期修改、生命周期和 revision 绑定。`PMAI_SESSION_EVAL_TIMEOUT_SECONDS` 默认 900 秒，和 runner 的 600 秒单例边界保持层级关系。真实批次使用已授权的 Codex CLI `0.148.0`、第三方 provider、模型 `gpt-5.6-sol` 和 reasoning effort `medium`；每个结果 provenance 均绑定该模型配置、框架 revision 与消费仓 revision。
...
- `final-validation.py` 对候选绑定的 implementation commit 创建 detached validation worktree，默认在 `implementation.root` 运行命令；若命令参数明确引用仓库根下、但不在实现根下的已存在验证路径（例如根下 `tests/`），只对该命令回到仓库根执行并在 artifact 留痕。新 project definition 机械拒绝 commands/web.start 重复选择 root，legacy recovery 只精确移除一次重复前缀并在 artifact 留痕。命令按项保存 exit code/log，test/typecheck 失败仍继续 build；只有前两项可由 PM 绑定当前 artifact 接受为 limited，production build 保持硬门。
- `finalize-work.py` 按 v4+ lifecycle 只补缺失机械项，并用 candidate binding、commit/source hash 和 audit 游标从语义检查、`final_check / landed / documenting` 准确续跑；最终化现在由 Verifier receipt + pre-landing audit binding + 独立 Judge receipt 组成，Judge 必须使用不同 run id 并绑定同一 evidence digest，主控降级时显式记录 `main-fallback`。统一 coverage helper 要求 checks-spec 机器 P0/P1 为零并逐项确认声明状态。runner 只提交当前模块状态和 audit 目录，合入后输出 main 模块恢复位置；prototype boundary、迁移和安全等其它语义判断不伪装成自动通过。
...
- builder profile 按项目定义、配置、本机可用性和当前主控推荐；Claude Code、Codex、OpenCode 可作为外部执行器，当前主控对应的同名工具不进入候选；native-child 是新 build 默认工具，native 仅表示 PM 明确选择或 child 不可用时的 `main-fallback`，两者都始终可选。Kimi Code 与 Cursor Agent 已退出新 build 工具面，旧合同仍可由兼容层读取；Gemini CLI 已退出构建工具面。验收 profile 仍后台生成，不进入开工卡。
...
- 在真实消费仓 dogfood `/pmai-feedback` 的聚焦模式与完整审计升级各一次，记录完成时间，核对固定快照、问题归属和交接 Prompt 能否避免框架仓重复全文冷读；再按完整主控支持范围评估 Claude Code 的精确当前会话定位，不为仅 Builder 的 OpenCode 或已移除宿主建设主控 session locator，也不提供猜测式降级。
