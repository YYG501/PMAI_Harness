...
- Claude Code 与 Codex 保留完整主控能力。OpenCode 只作为 `/pmai-build` 的外部 Builder，不执行本仓 Skill、不推进 lifecycle、不写验收通过或 landing；新 build 默认由当前主控派发 `native-child` Builder，当前会话直接构建只作 child 不可用时的 `main-fallback` 降级。
