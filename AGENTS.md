...
- `/pmai-*` 是 PMAI 的用户命令名。Codex skill 列表已暴露 `pmai-*` 时优先按该 skill 执行；如果当前 Codex runtime 没有 slash / skill UI，再按本仓 `skills/<command-without-pmai-prefix>/SKILL.md` 的步骤执行。
...
4. 脚本返回 0 只代表 B 步完成，不代表完整 `/pmai-init-project` 已完成。必须继续按 `skills/init-project/SKILL.md` 的 C/D 步执行：写 `docs/PRODUCT.md` 一句话定位、填 `docs/DESIGN.md`、尽量起 `prototype/` 主原型、按 skill 要求 commit，并输出 Next Up。
5. C/D 步完成后，消费仓根目录应生成 `AGENTS.md` 和 `CLAUDE.md`。后续在消费仓继续工作时，以消费仓自己的 `AGENTS.md` 为入口。
...
3. 需要脚本时走 `PMAI_HOME` 或 `~/.pmai`，不要把 framework 资产复制进消费仓。
4. 消费仓中不要再跑 `/pmai-init-project`；起模块工作走 `/pmai-design`，推进走 `/pmai-build` / `/pmai-close` / `/pmai-status`。
