...
2. **放弃入库 LOG**：QUICKFIX_LOG.md 不入库，改用 `git log --grep` 反查 + `/pmai-quick-fix --history` 生成动态 view（§7）
...
框架当前规定：任何改动都必须走 `/pmai-new-req` → stage 1-7 → task 流程。对实质性需求这是合理的，但对**小改动**（错别字、格式、常量值、单个样式等）成本过高：
...
`/pmai-quick-fix` 是 **req 流程之外**的快捷路径。三条：
...
| 前次崩溃残留 | 下次 `/pmai-quick-fix` 启动时扫 `.worktrees/tmp-quick-*`，列给 PM。也可 `/pmai-quick-fix --cleanup` 批量清 |
...
`/pmai-quick-fix --skip-tsc "<desc>"`：跳过 tsc 检查。适用于：
...
**`/pmai-quick-fix --history [N]`**：打印最近 N 条（默认 10）quick-fix 记录：
```bash
...
**`/pmai-task-status` 增强**：无活跃 req 时显示"最近 3 条 quick-fix"；有活跃 req 时作为附加 section 显示。

### 7.4 入库 summary 文件（可选，默认关）

PM 如果坚持要一份 reviewable 入库文件，`/pmai-quick-fix --snapshot` 可以**按需生成** `QUICKFIX_LOG.md`：

```bash
/pmai-quick-fix --snapshot   # 调用 git log，生成 QUICKFIX_LOG.md，一次性 commit
```
...
## 8. 与 /pmai-new-req 的分界

skill **不**替 PM 判断，由 PM 自行判断：

| 场景 | 建议 |
|------|------|
| 错别字、标点、格式 | `/pmai-quick-fix` |
| 补一段说明、加一个链接 | `/pmai-quick-fix` |
| 改常量值、单个样式 | `/pmai-quick-fix` |
| 删少量死代码 | `/pmai-quick-fix` |
| 改 1-2 行逻辑（你确信） | `/pmai-quick-fix` |
| 跨多文件的功能改动 | `/pmai-new-req` |
| 新增一个完整模块 | `/pmai-new-req` |
| 实质性改架构决策 | `/pmai-new-req` |
| 需要自审（/qa、/review、/design-review）才放心 | `/pmai-new-req` |
| 需要 PM 仪式感地验收 | `/pmai-new-req` |
...
| `templates/CLAUDE.md.tmpl` | 在"分支结构"节声明 /pmai-quick-fix 是"唯一例外"；在"gstack"节或单独列出 /pmai-quick-fix 可用 |
...
可选：
  ...
  2. 放弃本次 quick-fix：/pmai-quick-fix --cancel tmp-quick-<ts>-<pid>
```
...
- 也可显式 `/pmai-quick-fix --cleanup`：批量清理所有 `tmp-quick-*` worktree（有未 commit 改动的 warn 再清）
...
### 10.6 `/pmai-quick-fix --cancel <branch>` 和 `--cleanup` 的 spec（v2 新增）
...
10. 同秒并发两次 `/pmai-quick-fix` → PID 后缀区分，两次独立跑通
...
15. `/pmai-quick-fix --cancel tmp-quick-xxx` → 指定 branch 清理
16. `/pmai-quick-fix --cleanup` → 批量清理残留
...
| 6 | `templates/CLAUDE.md.tmpl` 增声明 /pmai-quick-fix 例外 |
| 7 | 测试 17 场景（见 §11） |
...
| 6 | 历史怎么留？ | 两 commit + `[quick-fix]` 前缀 + git log 反查 + `/pmai-quick-fix --history` 动态 view | 不建新目录，git 本身是 authoritative 源。放弃入库 LOG（v1 方案）避免冲突磁铁和 hash 自引用 |
| 7 | 跟 /pmai-new-req 怎么分？ | 给指南表，不给硬规则 | 判断交给 PM |
