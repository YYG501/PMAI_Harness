...
- 模式 1：`/pmai-prd-writing --req <req-id>` 写本 req 的评审 PRD
- 模式 2：`/pmai-prd-writing --module <module-name>[,<module>]` 在主分支写一个/多个模块的 PRD
- 模式 3：`/pmai-prd-writing` 走交互问 PM 想写什么范围
...
- **取舍**：现做的成本就是跑一次 `/pmai-prd-writing --req <id>`——这次任务讨论已经表明 PM 在主分支也常这么用，不是新负担。
...
3b. rewrite 失败 / PM 拒绝 / 中断：
    ...
    $ exit 1（保持 req stage=7、req 留在 active/、PM 可改后重试 /pmai-close-req）
```
...
> 自动生成。改 quickfix 请用 `/pmai-quick-fix`；改本文件无效。
