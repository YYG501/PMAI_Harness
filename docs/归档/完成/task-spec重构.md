...
| `req-stage-gate` | **三 delta 顺序改不同 stage**（§X Round 3 补登，原 D7-7/D9-10 deferred 项收口）：delta-2+4 vp-4a 改 Stage 2→3 换芯（umbrella step 2）→ delta-8 vp-5 接 Stage 4→5 编排 `/pmai-implementation-design`（step 5）→ delta-9 vp-5 改 stage 4（拆「必跑 gap-check / 可选 DESIGN.md 更新」，step 6）。各 delta 顺序 patch 前一个的结果、无冲突；实施按 umbrella §8 step 顺序 |
