...
> 状态：架构边界已由 PM 确认；复杂度收口批次一至五已经落地。所有者视图中的收口序列已完成，不把未迁移的兼容资产写成已经清理。
>
...
五个收口批次完成后，仍需持续控制的维护成本是：

- 21 个公开 Skill 同时包含主链、专项、运维和恢复意图，职责分类已经建立，但个人真实使用频率仍需按具体候选补证据。
- lifecycle、旧 stage、顶层状态、build 内状态和多代合同仍需兼容读取，但主逻辑已经统一接收 canonical model。
- 主控已收缩为 Claude Code / Codex，但 Kimi / OpenCode 的旧主控资产仍处于诊断和清理兼容窗口。
- 飞书评审、原子写入等专项子系统维护成本高；产品等级和威胁边界已经明确，后续要防止再次越界扩张。
- 兼容资产已有统一读取边界和退休条件，但已知消费仓迁移尚未执行完。
- 关键实现和 Skill 文档体积较大，框架行为越来越难由所有者直接审阅。

复杂度收口批次已经完成，本文继续作为框架所有者全景，不再承担开放待办。接下来优先做真实消费仓 dogfood、已知消费仓受控迁移和兼容退出证据；是否建设 Session Eval、多 Agent 或动态工作流仍需按独立问题重新判断，不能自动续成下一批架构层。
...
3. 当前复杂度主要从哪里产生。
4. 哪些架构边界需要 PM 决策，以及这些决定当前如何落地。

本文已经记录 PM 确认的产品支持范围和风险边界，五个收口批次均已落地。以下事项没有被本轮默认为完成：

- 删除或隐藏具体 Skill；当前没有明确候选。
- 清理真实安装态中的 Kimi / OpenCode 遗留主控入口，以及迁移已有消费仓。
- 退休某个旧合同或消费仓版本的具体日期。
- 删除超出威胁模型的文件安全代码。
- 继续拆分已经隔离到 adapter 后的飞书内部实现。

这些不是收口批次遗留任务。当前开放验证只在 `RUNTIME.md` 维护；兼容迁移与退休条件以配套清单为准，任何实际删除仍需单独证据和 PM 确认。
...
本次盘点曾发现一处可见漂移：`RUNTIME.md` 把已经进入当时 `HEAD` 的修复写成“尚未提交”。批次一已修正文案并明确单一真相源；仍应让机器可派生的 Git 和测试事实尽量由检查生成或校验。
...
这里不复制会随提交漂移的行数、测试套件数或通过数；当前测试基线只见 `RUNTIME.md`。所有者需要持续关注的是以下相对维护面：

| 维护面 | 所有者关注点 |
|---|---|
| 公开 Skill 表面 | 主链、专项、运维和恢复入口是否仍对应独立用户意图 |
| Build 与恢复运行时 | 当前合同是否保持唯一写模型，旧版本是否只停留在读取边界 |
| 飞书 review / sync-from-lark / publish | 可选能力是否继续通过 adapter 与核心生命周期隔离 |
| 原子写入与宿主配置事务 | 实现是否仍限定在 ADR-004 的明确威胁模型内 |
| 测试与 Session Eval | 确定性合同是否稳定，真实效果证据是否被清楚区分 |
...
开放 dogfood 已统一进入 `RUNTIME.md`；兼容读取者、消费仓使用情况和退休条件由配套兼容清单维护。兼容代码只能按清单证据退出，不能从普通待办是否为空推导。
...
- 如果直接叠加 Loop Contract，容易形成另一套并行状态。

已落地的直接解法不是重写消费仓，而是在框架读取边界输出唯一 canonical model；主逻辑不再感知旧版本，写入只产生当前版本。
...
飞书相关实现和 Skill 维护远端 revision、原生结构、评论动作、handoff、checkpoint、回读验证和多个 schema 版本，是当前最重的可选子系统之一。

这些设计对文档评审安全有直接价值，但其故障面、测试成本和兼容成本已经与主链不同。PM 已确认飞书是官方维护的可选能力：三个公开意图继续保留，但内部 schema、环境和发布验证需要与核心完整性隔离。

### 7.5 文件安全实现超过常规单人工具复杂度

原子写入和宿主配置事务覆盖 symlink、目录 fd、inode、设备、CAS、并发替换、回滚和 Bash 3.2，维护面明显高于普通单人工具。
...
当前较大的实现集中在 `lark-review.py`、`atomic_file.py`、`build-contract.py`、`consumer-doctor.py` 和 `status-view.py`；主要前台 Skill 则集中在 build、design 和 spec-writing。具体行数随提交变化，不在本文维护。
...
`RUNTIME.md` 已经是状态真相源；本次盘点发现并修正过“修复尚未提交”与 Git `HEAD` 不一致的历史实例。根因不是缺少规则，而是状态字段需要人工重复维护。
...
| 扩展治理 | 已收口 | 宿主等级、飞书产品等级、内部状态归一化与 Loop Contract 已制度化 |
| Agent 真实效果证明 | 尚不完整 | 确定性测试强，真实 Session 的独立观测和稳定性评测尚未建立 |

综合判断：PMAI 已经是合格的确定性工作流 Harness，五个批次完成了本轮认知与架构收口。当前重点应是用真实消费仓验证这些边界并执行受控迁移，而不是继续横向增加能力。

## 9. 已确认的架构边界

以下按 ADR 形式记录。ADR-001 保持阶段性决定；ADR-002 至 ADR-006 已由 PM 在 2026-08-11 确认并落实到本轮收口。ADR-002 的新写入口收缩已经完成，但真实安装态和消费仓遗留资产仍按兼容清单受控迁移。
...
- **决定**：统一内部 Loop Contract，不新增大型用户能力。旧状态先归一化，再把 `观察 → 目标 → 允许动作 → 执行 → 验证 → 提交 / 恢复 / 停止` 定义为运行协议；lifecycle 继续是唯一持久状态。
- **实施结果**：Build、Proposal 与 Design 已映射到同一 Loop Contract，并由固定消费仓场景验证“走对、停对、恢复对”。比较 Prompt、动态路由或多 Agent 策略仍需另行进入 Session Eval。
- **保留方案**：继续让每个 Skill 独立描述自己的循环。
- **取舍**：统一协议降低重复和行为漂移，但需要对主链 Skill 和脚本职责重新分层。

## 10. 建议的收口顺序

执行时一次只推进一个批次。每批先完成清单与验收标准，再改实现；前一批没有形成可验证结果前，不并行启动后续大型重构。批次一至五已经完成，后续新增架构能力按独立问题重新判断，不自动续出“批次六”。
...
**状态：已完成（2026-08-12），也是所有者视图的最后一个收口批次。**

1. 新增 `skills/_shared/loop-contract.md`，固定每轮“恢复 → 确认目标 → 确认边界 → 执行最小完整动作 → 验证实际结果 → 路由”的顺序。
2. 统一七种非持久动作：`retry_current / await_pm_decision / route_proposal / route_design / advance / resume_checkpoint / complete`；它们只是当轮判断，不写入新的 loop state，lifecycle 仍是唯一持久状态。
3. Proposal、Design、Build 分别映射自己的输入、允许动作、验证、重试、上游升级和完成出口；`build-close` 只承接 `resume_checkpoint`，不再拥有平行收尾循环。
4. 新增 18 个固定消费仓合同场景，精确记录阶段、触发、动作、去向和 checkpoint 规则，覆盖“走对、停对、恢复对”；场景由确定性回归校验，并登记为 static eval。
5. 本批没有比较 Prompt、模型、动态路由或多 Agent 策略，也没有把静态合同回归冒充真实效果；这些问题仍需单独进入 Session Eval。

## 11. 现在不建议做什么

完成本轮收口后，以下能力仍不应仅以“更前沿”为理由进入默认主链：
...
这些技术可以保留为研究项，但在当前阶段加入，只会重新扩大已经收口的状态、入口和宿主矩阵。
...
5. **下一阶段**：内部合同和 Build Loop 收口已经完成；当前先做真实消费仓 dogfood 与受控迁移，暂不提前建设 Session Eval。
6. **安全边界**：按单人本地工具保护正常操作、合作并发和失败恢复，不防御同用户恶意进程与不合作 writer。

这些答案已经完成本轮实施。后续不再为机械清理、目录归一或文档漂移反复请求 PM 拍板；只有改变产品表面、支持承诺、数据风险或核心流程时再请求确认。
