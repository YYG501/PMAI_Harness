...
> 状态：Living document
> 最近核对：2026-08-11
> 已确认目标：Codex、Claude Code 为完整主控；Kimi Code、OpenCode 仅作为 Builder
> 落地状态：批次二已完成新写入口收缩；既有 Kimi/OpenCode 主控资产仍只诊断、不自动清理
...
| 外部 Builder adapter | `claude -p` | `codex exec` | `kimi --prompt` | `opencode run` |
...
| 能力 | Claude Code | Codex | Kimi Code | OpenCode |
|---|---|---|---|---|
| 产品等级 | 完整主控 + 可选 Builder | 完整主控 + 可选 Builder | 仅 Builder | 仅 Builder |
...
| 外部构建执行 | 保留 adapter | 保留 adapter | 保留 `kimi-code` adapter / profile | 保留 `opencode` adapter / profile |
...
Cursor Agent 不是主控候选，继续只作为 Builder；manual / native 表示当前主控亲自构建，不构成第五个宿主支持面。
...
flowchart LR
    ...
    C2 --> H
    H --> N["当前主控直接构建"]
    H --> B1["Kimi Code Builder"]
    H --> B2["OpenCode Builder"]
    H --> B3["Cursor Agent Builder"]
    H --> B4["另一完整主控的 Builder adapter"]
    B1 --> R["候选 diff + 退出码"]
    B2 --> R
    B3 --> R
    B4 --> R
...
3. Kimi / OpenCode 仍可被 `/pmai-build` 作为外部 Builder 选择和执行。
...
| Kimi Code | 仅 Builder | 新安装/升级/消费仓已收缩；遗留 Skill 与 managed hooks 只诊断、待显式清理 |
| OpenCode | 仅 Builder | 新安装/升级/消费仓已收缩；遗留全局/项目 commands 只诊断、待显式清理 |

当前产品承诺已经统一为两类：Claude Code/Codex 完整主控，Kimi/OpenCode/Cursor Agent 仅 Builder。任何仍写“Kimi / OpenCode 一等主控”的活跃文档或模板都属于漂移；若只存在于历史 CHANGELOG、归档或遗留消费仓，则按兼容证据保留，不作为当前用法。
