...
|---|---|---|---|---|---|---|
| 旧 `stage` | `scripts/_lib/state.py`、`scripts/status-view.py`、`scripts/skill-preamble.sh`、`scripts/consumer-doctor.py` | `build-contract.py` 仍随 lifecycle 写 `stage=1/2`，只作兼容展示 | `ExampleAgentProject`、`ExampleConsumerApp` 的 active work 均仍有 stage | 先让读取边界统一产出 canonical lifecycle；停止新写 stage；旧仓仅投影展示 | 当前版本停止写 stage；所有已知 active work 结束或升级；status / doctor / preamble 不再直接读 stage | `test-state-lib.sh`、`test-status-view.sh`、`test-build-contract.sh`、`test-consumer-doctor.sh` |
| 顶层 `lifecycle_state` 与 `build.lifecycle_state` | `state.py`、`status-view.py`、`consumer-doctor.py`、`build-contract.py` | v4 同时写顶层和 build 内状态 | 两个 active v4 消费仓都依赖双位置一致性 | 在单一 normalization 层校验并输出 canonical state；主逻辑不再自行回退 | 所有主逻辑只读 canonical state；写入位置完成单一化；旧 active work 清零 | `test-state-lib.sh`、`test-status-view.sh`、`test-build-contract.sh`、`test-consumer-doctor.sh` |
| build contract v1 | `build-contract.py` 默认缺版本为 v1；`consumer-doctor.py` 接受 1-4；旧 close 路径可读 | 新工作只写 v4 | 本次已知 active work 未发现 v1 | 仅允许历史 close / 诊断；无 active work 时回 design 生成新合同 | 已知仓与支持窗口内均无 v1 active work；历史 close 恢复入口有替代或明确停止支持 | `test-build-contract.sh`、`test-close-work.sh`、`test-consumer-doctor.sh` |
| build contract v2-v3 | `build-contract.py`、`status-view.py`、`consumer-doctor.py`；`finalize-work.py` 明确转交 `build-close` 恢复 | 新工作只写 v4 | 本次已知 active work 未发现 v2/v3 | 保留恢复，不升级进行中的合同；完成或取消后下一轮直接写 v4 | 支持窗口内 v2/v3 active work 为零；`build-close` 不再承担对应恢复 | `test-build-contract.sh`、`test-close-work.sh`、`test-land-work-v2.sh`、`test-consumer-doctor.sh` |
...
| `required_checks` 别名 | `build-contract.py`、`status-view.py`、`finalize-work.py`、`consumer-doctor.py` | v4 仍同时写 `final_checks` 和相同的 `required_checks` | 当前 v4 合同普遍存在，不是个别仓异常 | normalization 映射为 `final_checks`；先让新写停止重复字段 | 新写不再产生别名；所有支持内 active work 完成；无宿主或恢复路径直接读取别名 | `test-build-contract.sh`、`test-status-view.sh`、`test-finalize-work.sh`、`test-consumer-doctor.sh` |
...
| CLI `pmai status` | `pmai doctor --check` 的弃用兼容别名；不等于 `/pmai-status` | 外部 shell 调用未知 | 保留薄包装并持续打印弃用提示 | 已确认没有脚本或使用说明依赖；经过一个稳定发布窗口 | `test-doctor-skills.sh`、`test-init-project-codex-compat.sh`、`test-opencode-host-compat.sh` |
